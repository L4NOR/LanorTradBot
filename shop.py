...
class ShopSystem(commands.Cog):
    ...
    @commands.command(name="shop_edit")
    @commands.has_any_role(*ADMIN_ROLES)
    async def shop_edit_item(self, ctx, item_id: str, field: str = None, *, value: str = None):
        """Modifie un item de la boutique.
        Usage: !shop_edit <item_id> <champ> <valeur>
        Champs: name, price, description, emoji, category, stock, type, active, level
        Sans arguments: affiche les champs modifiables.
        """
        if item_id not in shop_items:
            # Peut-être que l'utilisateur a mis le nom au lieu de l'ID
            found = None
            for iid, idata in shop_items.items():
                if idata.get("name", "").lower() == item_id.lower():
                    found = iid
                    break
            if found:
                item_id = found
            else:
                await ctx.send(f"❌ Item `{item_id}` introuvable. Utilisez `!shop` pour voir les IDs.")
                return
        ...
        if not field:
            embed = discord.Embed(
                title=f"✏️ Modifier: {item.get('name', item_id)}",
                description=f"**ID:** `{item_id}`\n\nUtilisez `!shop_edit {item_id} <champ> <valeur>`",
                color=discord.Color.blue()
            )
            fields_info = {
                "name": item.get("name", "N/A"),
                "price": f"{item.get('price', 0)} pts",
                "description": item.get("description", "N/A")[:100],
                "emoji": item.get("emoji", "N/A"),
                "category": item.get("category", "N/A"),
                "stock": item.get("stock", -1),
                "type": item.get("type", "N/A"),
                "active": item.get("active", True),
                "level": item.get("requirements", {}).get("level", 0),
            }
            for fname, fval in fields_info.items():
                embed.add_field(name=f"`{fname}`", value=str(fval), inline=True)
            await ctx.send(embed=embed)
            return

        field = field.lower()
        editable_fields = ["name", "price", "description", "emoji", "category", "stock", "type", "active", "level"]

        if field not in editable_fields:
            await ctx.send(f"❌ Champ invalide. Champs disponibles: `{'`, `'.join(editable_fields)}`")
            return

        if not value:
            await ctx.send(f"❌ Veuillez fournir une valeur. Ex: `!shop_edit {item_id} {field} nouvelle_valeur`")
            return

        old_value = None

        # Appliquer la modification selon le type de champ
        if field == "price":
            try:
                new_val = int(value)
                if new_val < 0:
                    ...
                    return
                old_value = item.get("price", 0)
                item["price"] = new_val
            except ValueError:
                await ctx.send("❌ Le prix doit être un nombre entier.")
                return
        elif field == "stock":
            try:
                new_val = int(value)
                old_value = item.get("stock", -1)
                item["stock"] = new_val
            except ValueError:
                await ctx.send("❌ Le stock doit être un nombre (-1 = illimité).")
                return
        elif field == "active":
            if value.lower() in ("true", "oui", "1", "on"):
                old_value = item.get("active", True)
                item["active"] = True
            elif value.lower() in ("false", "non", "0", "off"):
                old_value = item.get("active", True)
                item["active"] = False
            else:
                await ctx.send("❌ Valeur invalide. Utilisez `true/false` ou `oui/non`.")
                return
        elif field == "level":
            try:
                new_val = int(value)
                if "requirements" not in item:
                    item["requirements"] = {"badges": [], "level": 0}
                old_value = item["requirements"].get("level", 0)
                item["requirements"]["level"] = new_val
            except ValueError:
                await ctx.send("❌ Le niveau doit être un nombre entier.")
                return
        else:
            # Champs texte : name, description, emoji, category, type
        ...
        embed = discord.Embed(
            title="✅ Item Modifié",
            description=f"**{item.get('name', item_id)}** a été mis à jour.",
            color=discord.Color.green()
        )
        ...
        embed.set_footer(text=f"ID: {item_id} • Modifié par {ctx.author.name}")
        await ctx.send(embed=embed)

    @commands.command(name="remove_item", aliases=["take_item"])
    @commands.has_any_role(*ADMIN_ROLES)
    async def remove_item(self, ctx, member: discord.Member, *, item_name: str):
        """Retire un item de l'inventaire d'un membre.
        Usage: !remove_item @membre <nom_ou_id_item>
        """
        inv = get_user_inventory(member.id)
        items = inv.get("items", {})

        # Chercher l'item par ID ou par nom
        item_id = None
        for iid, idata in shop_items.items():
            if iid == item_name.lower().replace(" ", "_") or idata.get("name", "").lower() == item_name.lower():
                item_id = iid
                break
        ...
        if item_id not in items or items[item_id] <= 0:
            await ctx.send(f"❌ {member.mention} ne possède pas l'item `{item_name}`.")
            return

        item_display = shop_items.get(item_id, {}).get("name", item_id)
        items[item_id] -= 1
        if items[item_id] <= 0:
            del items[item_id]

        sauvegarder_shop()
        await ctx.send(f"✅ **{item_display}** retiré de l'inventaire de {member.mention}.")
